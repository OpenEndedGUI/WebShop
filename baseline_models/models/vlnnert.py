...
class VisualBertModelForWebshop(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.image:
            self.visual_bert = VisualBertModel.from_pretrained("uclanlp/visualbert-vqa-coco-pre").to('cuda')
        
        self.text_bert = BertModel.from_pretrained('bert-base-uncased').to('cuda')
    ...
    def forward(self, state_input_ids, state_attention_mask, action_input_ids, action_attention_mask, sizes, images=None, labels=None):
        ...
        action_rep = self.visual_bert(action_input_ids, attention_mask=action_attention_mask)[0]
        ...
        # images = self.image_linear(images).unsqueeze(1) 
        # state_rep = self.bert(input_ids=state_input_ids,
        #                 attention_mask=state_attention_mask,
        #                 visual_embeds=images,
        #                 visual_attention_mask=torch.ones(images.shape[:-1]),
        #                 output_attentions=False,
        #             )[0]
        # action_rep = self.text_bert(action_input_ids, attention_mask=action_attention_mask)[0]
        # state_rep = torch.cat([state_rep[i:i+1].repeat(j, 1, 1) for i, j in enumerate(sizes)], dim=0)
