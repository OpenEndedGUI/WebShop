...
from models.vlnbert import VisualBertModelForWebshop
...
def get_data(split, mem=False, filter_search=True, images_only=False):
    ...
    # print("JSON_LIST keys:",result.keys()) #dict_keys(['actions', 'states', 'available_actions', 'actions_translate', 'action_idxs', 'images'])
    ...
    # # print("available_actions:",result['available_actions'])
    # # print("action_idxs:",result['action_idxs'])
    # # print("images:",result['images'])
    # # print("--------------------------")
    # # print("JSON_LIST:",len(json_list)) #1571
    # print("++++++++++++++++++++++++++")
    # assert False
    ...
    for json_str in json_list:
        result = json.loads(json_str) ### obs = result['states']
        s = process_goal(result['states'][0]) ##process goal
        ...
        for state, valid_acts, idx, image in zip(result['states'], result['available_actions'], result['action_idxs'], result['images']):
            ...
            if filter_search and idx == -1:
                continue
            if images_only:
                state_list.append(result['states'][0])
            else:  
    ...
    if images_only:
        state_list, action_list = list(
            map(process_goal, state_list)), list(map(process, action_list))
    else:
        state_list, action_list = list(
...
def get_dataset(split, mem=False, images_only=False):
    states, actions, idxs, sizes, images = get_data(split, mem, images_only=images_only)
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./ckpts/web_click_image_only",
...
def main():
    ...
    model = BertModelForWebshop(config).to('cuda')
    # model.bert.resize_token_embeddings(len(tokenizer))

    train_dataset = get_dataset("train", mem=args.mem, images_only=True)
    eval_dataset = get_dataset("eval", mem=args.mem, images_only=True)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # print("State:",batch["state_input_ids"].shape)
